...
// output:First Promise Executed

//Task2 Create a promise that rejects with a message after a 2 second timeout and handle the error using .catch()

 let first_promise_error=new Promise((resolve,reject)=>{
    setTimeout(()=> reject(`First Promise Failed`),2000);
})

first_promise_error.catch(()=> console.log('Error was there'));
// output `First Promise Failed`
...
first_promise.then((message)=>{
    ...
    return first_promise_error.catch((message)=>{
        console.log(message);
    })
})
//output
// Fetching from 1...
// First Promise Executed
// Fetching from 2...
// Error was there
// First Promise Failed
